...
units=metric
lang=es
forecast="api.openweathermap.org/data/2.5/forecast/daily?id=${city_id}&appid=${api_key}&cnt=6&units=${units}&lang=${lang}"
weather="api.openweathermap.org/data/2.5/weather?id=${city_id}&appid=${api_key}&units=${units}&lang=${lang}"
...
curl ${weather} -s -o ~/.cache/day-weather.json
curl ${forecast} -s -o ~/.cache/forecast-weather.json
# weather icon
# weather_code=$(jq -r '.weather[0].icon' ~/.cache/conky-weather.json)
# echo $weather_code
cp -r ~/.config/conky/icons/$(jq -r '.weather | .[].icon' ~/.cache/day-weather.json).png ~/.cache/conky-weather-icon.png
cp -r ~/.config/conky/icons/$(jq -r '.list | .[1] | .weather | .[].icon' ~/.cache/forecast-weather.json).png ~/.cache/conky-weather-icon1.png
cp -r ~/.config/conky/icons/$(jq -r '.list | .[2] | .weather | .[].icon' ~/.cache/forecast-weather.json).png ~/.cache/conky-weather-icon2.png
